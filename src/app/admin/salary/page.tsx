...
export default function SalaryPage() {
  ...
  const fetchSalaryData = async () => {
    try {
      ...
      const response = await fetch(`/api/salary?year=${selectedPeriod.year}&month=${selectedPeriod.month}`)
      const result = await response.json()
      
      if (!response.ok) {
        throw new Error(result.error || '給与データの取得に失敗しました')
      }
      ...
      if (result.success) {
        ...
        throw new Error(result.error || 'データの処理に失敗しました')
      }
    } catch (error) {
  ...
  }, [status, session, selectedPeriod, error])
